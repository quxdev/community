...
CHAR256 = {"max_length": 256, "default": None, "null": True, "blank": True}

dnb = dict(default=None, null=True, blank=True)
...
class Link(CoreModel):
    ...
    short_url = models.CharField(
        max_length=16, unique=True, default=None, null=True, verbose_name="Short URL"
    )
    ...
    def create_short_link(original_url, shorturl, expiry_date=None, domain=None):
        ...
        filterparams = dict(original_url=original_url, domain=domain)
        linkobj = None
        if shorturl:
    ...
    def get_original_url(request, slug):
        ...
        linkobj = (
            Link.objects.filter(short_url=slug)
...
class LinkVisit(CoreModel):
    ...
    http_accept_language = models.CharField(
        max_length=256, **dnb, verbose_name="HTTP Language"
    )
    http_user_agent = models.CharField(
        max_length=512, **dnb, verbose_name="HTTP UserAgent"
    )
    http_referer = models.URLField(max_length=1024, **dnb, verbose_name="HTTP Referer")
    remote_addr = models.CharField(max_length=256, **dnb, verbose_name="Remote Address")
    remote_port = models.CharField(max_length=256, **dnb, verbose_name="Remote Port")
    server_name = models.CharField(max_length=256, **dnb, verbose_name="Server Name")
    server_port = models.CharField(max_length=256, **dnb, verbose_name="Server Port")
    http_host = models.URLField(max_length=1024, **dnb, verbose_name="HTTP Host")
    query_string = models.URLField(max_length=1024, **dnb, verbose_name="Query String")
    ...
    def store_data(request, track_obj=None):
        ...
        [setattr(track_obj, k.lower(), request.META.get(k, None)) for k in metadata]
